      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-white/95 backdrop-blur-md shadow-md border-b border-navy/10 py-3" : "bg-white py-5"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center gap-6">
              <button onClick={() => navigateTo("home")} className={`text-sm font-medium ${currentRoute === "home" ? "text-[#1B2F6E] font-semibold" : "text-gray-500 hover:text-[#1B2F6E]"}`}>{t("nav_home")}</button>
              <div className="h-4 w-[1px] bg-[#1B2F6E]/20"></div>
              <div className="flex items-center gap-3">
                <button onClick={() => navigateTo("login")} className="px-5 py-2 text-sm font-medium border border-[#1B2F6E] text-[#1B2F6E] rounded-full hover:bg-navy/5">{t("nav_login")}</button>
                <button onClick={() => navigateTo("register")} className="px-6 py-2 text-sm font-semibold bg-[#F5A623] text-[#1B2F6E] rounded-full hover:shadow-lg">{t("nav_get_started")}</button>
              </div>
            </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white border-b border-navy/10 shadow-lg px-4 py-6 flex flex-col gap-4">
             <button onClick={() => { setIsMobileMenuOpen(false); navigateTo("home"); }} className="text-left py-2 font-medium">Home</button>
             <button onClick={() => { setIsMobileMenuOpen(false); navigateTo("login"); }} className="px-5 py-3 text-center border border-[#1B2F6E] text-[#1B2F6E] font-medium rounded-xl">Login</button>
             <button onClick={() => { setIsMobileMenuOpen(false); navigateTo("register"); }} className="px-5 py-3 text-center bg-[#F5A623] text-[#1B2F6E] font-bold rounded-xl">Get Started</button>
          </div>
        )}
      </nav>
    );
    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 bg-white shadow-sm transition-all duration-300 py-2.5 px-4 md:px-8 border-b border-gray-200`}>
        <div className="max-w-[1200px] mx-auto flex justify-between items-center">
          ...
          {/* LEFT: Logo */}
          <div onClick={() => navigateTo("feed")} className="cursor-pointer">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {isProfileDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden z-50">
                  ...
                  <div className="p-1">
                    <button onClick={() => { setIsProfileDropdownOpen(false); navigateTo("profile"); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-md">
                      <User className="w-4 h-4" /> আমার প্রোফাইল
                    </button>
                    <button onClick={() => { setIsProfileDropdownOpen(false); navigateTo("settings"); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-md">
